...
This repo uses a **deterministic, offline-safe** guardrail for container platform stacks.

- Guardrail ID: `gr-containers-terraform-baseline`
- Failure string: `guardrail unmet: cluster telemetry missing`
...
diag_blocks = list(re.finditer(r'resource\s+"azurerm_monitor_diagnostic_setting"\s+"[^\"]+"\s*\{(.*?)\}', text, re.S|re.I))
if diag_blocks:
    bound = False
...
secondary_to_primary = re.search(r'provider\s*=\s*azurerm\.secondary[\s\S]*?remote_virtual_network_id', text, re.I) or re.search(r'remote_virtual_network_id\s*=.*(primary|pri)', text, re.S|re.I)
...
if any([cluster_missing, telemetry_missing, diagnostic_binding_missing, reverse_link_missing, route_assoc_missing, tags_missing]):
    print(FAIL)
    print(f'issues: cluster_missing={cluster_missing}, telemetry_missing={telemetry_missing}, diagnostic_binding_missing={diagnostic_binding_missing}, reverse_link_missing={reverse_link_missing}, route_assoc_missing={route_assoc_missing}, tags_missing={tags_missing}')
    sys.exit(1)
